...
if(!class_exists('THWPSB_Db_Helper')):

/**
 * The wrapper class to help WordPress CRUD for a single table.
 *
 * @since      0.0.1
 * @package    ThemeHighDbWrapper
 * @subpackage ThemeHighDbWrapper\THWPSB_Db
 * @author     Lucis Lynn <ThemeHighDbWrapper@gmail.com>
 *
 * How To Use
 *
 * $mydb = new ThemeHighDbWrapper\Db( 'mytable_without_wpdb_prefix' );
...
class THWPSB_Db_Helper {
     ...
     */
    public function __construct( $tablename, $type = false ) {

        global $wpdb;

        if( is_null( $this->wpdb ) ) {
            $this->wpdb    = $wpdb;
            if($type = 'multisite_main' ){
     ...
     */
    public function get_all( $orderby = NULL, $order = NULL, $per_page = false, $page_number = false, $output_type = OBJECT ) {
	...
	/**
     * Get year and month group of a datetime column in $this->tablename;
     * @since  0.0.1
     * @return array of year month
     */
    public function get_year_month_column($column, $output_type = OBJECT) {
		$column = $this->check_column( $column );
		$sql = "SELECT YEAR($column) AS 'year', MONTH($column) AS 'month' FROM $this->tablename GROUP BY YEAR($column), MONTH($column)"; //WHERE";
	...
	// public function get_sb_count_by_datetime(){
	//
	// }
